    ...
    for _, entity_type in pairs(applicable_entity_types) do
        for _, entity in pairs(data.raw[entity_type]) do
            if name_check(entity.name, target_race) == true then
                if entity.graphics_set and entity.graphics_set.animations then
                    entity.graphics_set.animations = AnimationDB.alter_team_color(entity.graphics_set.animations, color, disable_mask, preserve_gloss)
                end
                if entity.animation then
                    entity.animation = AnimationDB.alter_team_color(entity.animation, color, disable_mask, preserve_gloss)
                end
